...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!symbols.length) return new Response(JSON.stringify({ok:false, code:'MISSING_SYMBOLS'}), {status:400});
  
  const base = symbols[0].toUpperCase();
  const r = await (await eodFetch(`/real-time/${encodeURIComponent(base)}`, { 
