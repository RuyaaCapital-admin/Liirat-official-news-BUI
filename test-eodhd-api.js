...
async function testEODHDAPI() {
  ...
  try {
    ...
    const calendarResponse = await fetch('/api/eodhd-calendar?limit=10&importance=3,2', {
      method: 'GET',
      headers: {
    ...
    if (calendarResponse.ok) {
      ...
      console.log('Number of events received:', calendarData.events?.length || 0);
      console.log('Sample event data:', calendarData.events?.[0] || 'No events');
      ...
      console.log('❌ Calendar API failed with status:', calendarResponse.status);
...
if (typeof window !== 'undefined') {
  ...
  console.log('🔧 EODHD API test function loaded. Run window.testEODHDAPI() in console to test.');
