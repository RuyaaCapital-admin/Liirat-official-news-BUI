...
export function AIEventInsight({ event, className }: AIEventInsightProps) {
  ...
  const generateDemoAnalysis = (
    ...
    const isAboveExpected =
      parseFloat(eventData.actual) > parseFloat(eventData.forecast);
  ...
  const fetchAIInsight = async () => {
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "Summarize economic events in a single, short, direct sentence. Maximum 25 words. No explanations, no introduction, no extra details. Write ONLY the most actionable insight for a trader. Match the requested language exactly.",
            },
            {
              role: "user",
              content: prompt,
      ...
      setError(
        err instanceof Error ? err.message : "خطأ في الحصول على التحليل",
      );
    } finally {
      setIsLoading(false);
    }
