...
export function CustomDatePicker({ value, onValueChange, className }: CustomDatePickerProps) {
  ...
  const presetOptions = [
    ...
    { 
      ...
      getDate: () => ({ from: startOfWeek(new Date()), to: endOfWeek(new Date()) })
    },
    { 
      value: 'next-week', 
      label: t('date.nextweek'),
      getDate: () => ({ from: startOfWeek(addDays(new Date(), 7)), to: endOfWeek(addDays(new Date(), 7)) })
    },
    { 
      value: 'this-month', 
      label: language === 'ar' ? 'هذا الشهر' : 'This Month',
      getDate: () => ({ from: startOfMonth(new Date()), to: endOfMonth(new Date()) })
    }
  ];
