...
interface AlertContextType {
  ...
  clearAllAlerts: () => void;
  addPreference: (
    preference: Omit<AlertPreference, "id" | "createdAt">,
  ) => void;
...
export function AlertProvider({ children }: AlertProviderProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedPreferences) {
        const parsedPreferences = JSON.parse(savedPreferences).map(
          (pref: any) => ({
            ...pref,
  ...
  const addAlert = (
    alertData: Omit<ActiveAlert, "id" | "timestamp" | "isRead">,
  ) => {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification(
        "Liirat - " + (language === "ar" ? "تنبيه اقتصادي" : "Economic Alert"),
        {
  ...
  const addPreference = (
    preferenceData: Omit<AlertPreference, "id" | "createdAt">,
  ) => {
