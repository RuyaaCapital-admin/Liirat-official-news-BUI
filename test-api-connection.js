...
async function testAPIConnection() {
  ...
  try {
    ...
    const calendarResponse = await fetch('/api/eodhd-calendar?limit=5&importance=3', {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
...
if (typeof window !== 'undefined') {
  ...
  console.log('🔧 API test function loaded. Run window.testAPIConnection() in console to test.');
}
