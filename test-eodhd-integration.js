...
async function testEndpoint(endpoint) {
  try {
    ...
    if (foundMockIndicators.length > 0) {
      console.log(
        `   ❌ Found mock data indicators: ${foundMockIndicators.join(", ")}`,
      );
      return false;
    }
    ...
    if (data.message && data.message.includes("EODHD API access restricted")) {
      console.log(
        `   ✅ EODHD API access restricted message found (real API call)`,
      );
      return true;
    }
    ...
    if (
      ...
      console.log(
        `   ✅ Empty data with proper structure (likely API restrictions)`,
      );
      return true;
    }
  ...
  try {
    ...
    for (const message of testMessages) {
      ...
      if (data.response) {
        ...
        if (foundMockIndicators.length > 0) {
          console.log(
            `   ❌ Response contains mock data indicators: ${foundMockIndicators.join(", ")}`,
          );
          return false;
        }

        console.log(
          `   ✅ Chat response looks clean (no mock data indicators)`,
        );
      }
    }
